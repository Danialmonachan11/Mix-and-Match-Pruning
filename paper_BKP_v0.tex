...
Pruning methods have evolved from magnitude-based pruning (MAG) \cite{han2015learning} that removes smallest-magnitude weights, to Single-shot Network Pruning (SNIP) \cite{lee2018snip} using gradient-weight products, and Gradient Signal Preservation (GraSP) \cite{wang2020picking} incorporating Hessian information to preserve gradient flow. More recently, specialized methods like LPViT \cite{wu2025lpvit} for low-power vision transformers and GETA \cite{geta2025} for joint pruning-quantization have been proposed to address specific architectural challenges. Each of these represents a different pruning criterion—that is, a rule for deciding which weights to remove, such as weight magnitude, gradient magnitude, or gradient flow preservation. These single-shot methods achieve competitive results compared to iterative pruning approaches with minimal computational overhead.
...
\begin{enumerate}
    \item \textit{Magnitude-only:} \quad $S_i^{(l)} = |w_i^{(l)}|$ \cite{han2015learning}
    \item \textit{Gradient-only:} \quad $S_i^{(l)} = \left|\frac{\partial \mathcal{L}}{\partial w_i^{(l)}}\right|$ \cite{lee2018snip}
    \item \textit{Magnitude--gradient product:} \quad $S_i^{(l)} = |w_i^{(l)}| \cdot \left|\frac{\partial \mathcal{L}}{\partial w_i^{(l)}}\right|$ \cite{wang2020picking}
\end{enumerate}

These formulations quantify weight saliency from complementary perspectives: static structural relevance (magnitude)~\cite{han2015learning}, dynamic training sensitivity (gradient)~\cite{lee2018snip}, and a combined measure reflecting both~\cite{wang2020picking}. Gradients are accumulated over a held-out calibration set and normalized to ensure comparability across layers. After computing $S_i^{(l)}$ for all weights, weights within each layer are ranked, and the lowest-ranked $\rho^{(l)}$ proportion is pruned using binary masks:
...
\begin{algorithmic}[1]
\STATE \textbf{Input:} Pretrained network $\mathcal{N}$, pruning baselines $\mathcal{B}$, sparsity targets $\mathcal{T}$, pruning criteria $\mathcal{C}$
...
\begin{figure*}[!tb]
   ...
   \caption{Sparsity vs. accuracy trade-offs for Mix-and-Match pruning across four architectures. Blue circles represent the 10 generated strategies, red squares show baseline methods (MAG, SNIP, GraSP, LPViT, GETA), and green dashed lines indicate unpruned baseline accuracy. The framework produces a diverse set of solutions spanning the Pareto front, with Mix-and-Match strategies consistently outperforming baselines at comparable sparsity levels. (a) VGG-11 on CIFAR-10, (b) ResNet-18 on GTSRB, (c) LeViT-384 on CIFAR-10, (d) Swin-Tiny on CIFAR-100.}
...
The results indicate that the optimal sensitivity metric depends on the architecture: a combination of magnitude and gradient is most effective for VGG-11, magnitude alone for ResNet-18 and Swin-Tiny, and gradient alone for LeViT-384.
...
X. Wu, Y. Zhang, and Q. Li, ``LPViT: Low-Power Semi-structured Pruning for Vision Transformers,'' in \textit{European Conference on Computer Vision (ECCV)}, 2025.
\bibitem{geta2025}
A. Author et al., ``GETA: Joint Pruning and Quantization,'' in \textit{CVPR}, 2025.
\end{thebibliography}
