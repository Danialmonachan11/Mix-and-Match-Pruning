...
%We contribute: (1) a general framework that establishes architecture-aware sparsity ranges from K baseline methods (demonstrating K=3: MAG, SNIP, GraSP) and generates diverse strategies through systematic range exploration, extensible to alternative baseline method combinations; (2) multi-point pruning solutions (10 operating points per architecture) enabling deployment flexibility without re-execution of pruning procedures, contrasting with traditional single-point optimization approaches; (3) comprehensive evaluation revealing when multi-strategy exploration succeeds (VGG-11, ResNet-18, LeViT-384) versus when single-method baselines are superior (Swin-Tiny), providing empirical guidance for method selection; (4) architecture-specific insights identifying that hierarchical transformers with structural dependencies require global coordination, while CNNs and hybrid transformers benefit from layer-wise exploration.

% pruning methods typically produce single-point solutions tuned to a specific sparsity target, requiring full re-execution when deployment scenarios demand different accuracy--compression trade-offs. In addition, architectural components such as convolutional layers, residual connections, and attention mechanisms respond differently to parameter removal: skip connections necessitate conservative pruning to maintain gradient flow, while self-attention relies on globally distributed dependencies that constrain achievable sparsity. As a result, current single-point approaches are unable to systematically explore this heterogeneous design space, and empirical evidence for multi-strategy pruning in vision transformers remains limited. A key unresolved challenge is the selection of an appropriate pruning criterion for a given model, based on its structure and parameter characteristics. This gap motivates a mix-and-match pruning paradigm that determines suitable strategies for each architecture through coordinated multi-criterion exploration.
% this gap, a multi-strategy pruning framework is proposed that systematically explores families of solutions by observing baseline pruning behavior to establish architecture-aware sparsity ranges, then generating diverse strategies spanning the accuracy--sparsity trade-off space. When combined with post-training quantization, the framework provides deployment flexibility through multiple operating points without re-executing pruning procedures, producing multiplicative compression gains suitable for edge-oriented scenarios.
% contributions of this paper are as follows.}
%}%  Mix-and-Match Pruning is introduced that establishes architecture-aware sparsity ranges from multiple baseline criteria (demonstrated with three representative methods) and generates diverse layer-adaptive strategies through systematic exploration, while remaining extensible to othe% Multi-point pruning outputs are produced, offering multiple operating points per architecture and enabling deployment flexibility without re-executing the pruning procedure, in contrast to traditional single-point approa% A comprehensive evaluation is conducted across CNNs and vision transformers, revealing when multi-strategy exploration is advantageous and when individual baseline methods perform comparably.
%{itemize}
% of this paper is organized as follows. Section~\ref{sec:method} presents the proposed methodology. Section~\ref{sec:results} reports the experimental results and analysis and Section~\ref{sec:conclusion} concludes the paper.


Deep neural networks (DNNs) achieve state-of-the-art performance across vision tasks but their deployment on edge devices is hindered by strict memory ($<10 MB$), compute, and power constraints \cite{}. Modern CNNs often exceed 100 MB and Vision Transformers (ViTs) reach several hundred megabytes, making direct deployment impractical \cite{}. While compression techniques such as quantization~\cite{jacob2018quantization}, knowledge distillation~\cite{hinton2015distilling}, and neural architecture search~\cite{zoph2017neural} offer partial solutions, pruning remains particularly effective due to its ability to remove redundant parameters while preserving accuracy.

Early pruning research focused on global magnitude-based criteria (MAG)~\cite{han2015learning}. Later works such as SNIP~\cite{lee2018snip} and GraSP~\cite{wang2020picking} incorporated gradient and Hessian information to estimate weight importance in a single-shot fashion. The Lottery Ticket Hypothesis~\cite{frankle2019lottery} further highlighted the existence of performant sparse subnetworks within dense models. In parallel, ViT compression has explored attention pruning~\cite{michel2019heads}, hybrid efficient architectures~\cite{liu2021swin,graham2021levit}, and post-training quantization~\cite{jacob2018quantization}. Despite these efforts, unified post-training pruning frameworks for both CNNs and transformers are still limited.

A core limitation of existing pruning methods is that they produce only a single sparsity point tuned to a predetermined compression target. When deployment requirements change, the entire pruning procedure must be re-executed. Furthermore, architectural components behave differently under pruning: convolutional layers and residual blocks require conservative pruning to preserve gradient flow, while self-attention layers rely on globally distributed dependencies that restrict achievable sparsity. As a result, no single pruning criterion consistently performs well across architectures or sparsity levels.

These challenges are amplified in layer-wise pruning, where determining the appropriate sparsity for each layer is crucial. Layer-wise strategies offer several advantages—improved accuracy retention, better alignment with hardware structure, and finer control over parameter removal—but they also expose substantial variability across architectures. Selecting a suitable pruning criterion therefore becomes a non-trivial task, especially in post-training scenarios.

This gap motivates a multi-strategy, layer-wise pruning paradigm. Instead of committing to a single criterion, we seek to systematically explore multiple global pruning metrics, derive architecture-aware sparsity ranges, and generate diverse layer-adaptive strategies that collectively cover a broad accuracy--sparsity trade-off space.

% address this need, we propose Mix-and-Match Pruning}, a mult% baseline pruning behavior from multiple global criter% architecture-aware layer-wise sparsity ranges, and  % a family of pruned models offering multiple deployment-ready operating points without re-running pruning.
% with post-training quantization, the framework enables flexible and multiplicative compression suitable for edge devices.
